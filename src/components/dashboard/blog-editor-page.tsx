  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      if (file.size > 20 * 1024 * 1024) {
        toast.error("File too large — max 20 MB before compression");
        return;
      }
          ...
          <div className="relative group rounded-lg overflow-hidden aspect-video bg-muted">
            ...
            <div className="absolute inset-0 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity bg-background/60 backdrop-blur-sm">
              <button
                ...
                className="px-2.5 py-1.5 rounded-lg bg-card border border-border text-foreground text-[11px] hover:bg-muted transition-colors"
              >
                Replace
              </button>
              <button
          ...
          {uploading ? (
            ...
            <>
              ...
              <span className="text-[11px] text-muted-foreground">
                — drag & drop or click · auto-compressed
              </span>
            </>
          )}
        </div>
...
function TableMenu({ editor }: { editor: ReturnType<typeof useEditor> }) {
  ...
  const actions = inTable
    ? [
        ...
        {
          label: "Add row below",
          fn: () => runAction(() => editor.chain().focus().addRowAfter().run()),
        },
        {
          label: "Delete row",
          fn: () => runAction(() => editor.chain().focus().deleteRow().run()),
          danger: true,
        },
        null, // separator
        {
        ...
        {
          label: "Delete table",
          fn: () => runAction(() => editor.chain().focus().deleteTable().run()),
          danger: true,
        },
      ]
    : [
  ...
  const addYoutube = () => {
    ...
    if (url)
      editor
        .chain()
        .focus()
        .setYoutubeVideo({ src: url, width: 720, height: 405 })
        .run();
  };
...
function SettingsPanel({
  ...
  const inputCls =
    "w-full bg-muted border border-border rounded-lg px-3 py-2 text-[13px] text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-ring transition-colors resize-none";

  const usedKb = MAX_POST_IMAGES_KB - budget.remainingKb;
  const usedPct = Math.min(
    100,
    Math.round((usedKb / MAX_POST_IMAGES_KB) * 100),
  );
    ...
    <div className="flex flex-col gap-0 divide-y divide-border">
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-1.5 bg-muted border border-border rounded-lg px-3 py-2">
          ...
          <input
            value={form.slug}
            onChange={(e) =>
              onChange({ slug: slugify(e.target.value), slugManual: true })
            }
      ...
      <div className="p-4">
        ...
        <CoverUpload
          ...
          onChange={(url, path) =>
            onChange({ cover_image_url: url, cover_storage_path: path })
          }
          budget={budget}
        />
      </div>
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          >
            {(usedKb / 1024).toFixed(2)} /{" "}
            {(MAX_POST_IMAGES_KB / 1024).toFixed(0)} MB
          </span>
        </div>
        ...
        <p className="text-[10px] text-muted-foreground mt-1.5">
          Cover + inline images for this post, after auto-compression (~1200px
          JPEG). Uploaded to storage only when you publish — drafts stay local.
        </p>
      </div>
...
export function BlogEditorPage({ post }: BlogEditorPageProps) {
  ...
  const materializeImages = async (): Promise<{
    cover: string;
    content: string;
  }> => {
    ...
    const dataSrcs = Array.from(
      new Set(extractImageSrcs(content).filter(isDataUrl)),
    );
  ...
  const publishMutation = useMutation({
    mutationFn: async () => {
      ...
      if (
        cover !== formRef.current.cover_image_url ||
      ...
      setIsPublished(true);
      toast.success(
        "Post published — now live at /blog/" + (form.slug || "untitled"),
      );
    },
                  ...
                  <div className="flex items-center gap-2 mb-8 pb-6 border-b border-border">
                    <span className="text-[11px] text-muted-foreground font-mono">
                      ...
                      <span className="text-foreground">
                        {form.slug || slugify(form.title) || "your-slug"}
                      </span>
                    </span>
                  </div>
