    ...
    if (type === "tool-createDocument") {
      ...
      const outputObj =
        part.output && typeof part.output === "object"
          ? (part.output as Record<string, any>)
          : null;

      if (outputObj && "error" in outputObj) {
        return (
          <div
            className="rounded-lg border border-red-200 bg-red-50 p-4 text-red-500 dark:bg-red-950/50"
            key={toolCallId}
          >
            Error creating document: {String(outputObj.error)}
          </div>
        );
      }
      ...
      if (outputObj && "error" in outputObj) {
        return (
          <div
            ...
            key={toolCallId}
          >
            Error updating document: {String(outputObj.error)}
          </div>
        );
      }
